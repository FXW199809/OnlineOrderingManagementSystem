...
import com.HotPot.service.liuService;
...
public class goods_servlet extends HttpServlet
{
	...
	//添加商品
	{
		...
		String sql="insert into t_goods(id,catelog_id,bianhao,mingcheng,fujian,price,del) " +
				   "values(?,?,?,?,?,?,?)";
		Object[] params={id,catelog_id,bianhao,mingcheng,fujian,price,del};//Object[]定义一个一维对象，不需要强制类型转换
		DB mydb=new DB();
	...
	//商品信息编辑
	{
		...
		String sql="update t_goods set catelog_id=?,bianhao=?,mingcheng=?,fujian=?,price=? where id=?";
		Object[] params={catelog_id,bianhao,mingcheng,fujian,price,id};
		DB mydb=new DB();
		mydb.doPstm(sql, params);
		mydb.closed();
